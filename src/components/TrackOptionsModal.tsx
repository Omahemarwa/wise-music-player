...
export const TrackOptionsModal: React.FC<TrackOptionsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/65 backdrop-blur-sm p-3">
      <div className="w-full max-w-sm bg-[#1c1a26] border border-white/10 rounded-3xl p-5 shadow-2xl space-y-4 animate-in fade-in slide-in-from-bottom-6">
        ...
        <div className="flex items-center gap-3 pb-3 border-b border-white/10">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-[#958da1] truncate mt-0.5">{track.artist}</p>
            <div className="flex items-center gap-2 text-[10px] text-[#c7bfff] font-mono mt-1">
        ...
        {!showPlaylistsPicker ? (
          <div className="space-y-1 text-sm">
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 text-white transition-colors cursor-pointer text-left"
            >
              <Heart
                className={`w-5 h-5 ${
                  track.isFavorite ? 'fill-[#c7bfff] text-[#c7bfff]' : 'text-[#c7bfff]'
                }`}
              />
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 text-white transition-colors cursor-pointer text-left"
            >
              <FolderPlus className="w-5 h-5 text-[#c7bfff]" />
              <span>Add to Playlist...</span>
            </button>

            {/* File Path Info */}
            <div className="px-3 py-2 text-xs text-[#958da1] bg-[#12111a] rounded-xl border border-white/5 space-y-1">
              <div className="flex items-center gap-1.5 text-white/90">
                <Info className="w-3.5 h-3.5 text-[#c7bfff]" />
                <span className="font-semibold">Offline File Info</span>
              </div>
              <p className="truncate">Path: <span className="font-mono text-white/70">{track.folderPath}</span></p>
              <p>Bitrate: <span className="text-white/70">{track.bitrate || 'Standard Audio'}</span></p>
              <p>Play count: <span className="text-white/70">{track.playCount}</span></p>
            </div>
            ...
            {onDeleteTrack && (
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-red-500/10 text-red-400 hover:text-red-300 transition-colors cursor-pointer text-left"
              >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs text-[#c7bfff] hover:underline"
              >
                Back
              </button>
            </div>
              ...
              {playlists.map((playlist) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2.5 rounded-xl hover:bg-white/5 text-left text-xs text-white border border-white/5"
                  >
                    ...
                    {isJustAdded ? (
                      ...
                      <span className="text-[#958da1]">Already added</span>
                    ) : (
